...
<script>
  import {fetchList,createAdmin,updateAdmin,updateStatus,deleteAdmin,getRoleByAdmin,allocateRole} from '@/api/user';
  ...
  export default {
    ...
    data() {
      return {
        ...
        total: null,
        listLoading: false,
        dialogVisible: false, //用户编辑弹窗是否显示
        admin: Object.assign({}, defaultAdmin),
        isEdit: false, // 判断是添加还是修改，false添加
        allocDialogVisible: false, //用户权限编辑弹窗是否显示
        allocRoleIds:[], // 用户拥有的全部权限
        allRoleList:[], // 可配置的全部权限
        allocAdminId:null // 当前配置权限的用户ID
      }
    },
    created() {
      this.getList();
    ...
    methods: {
      ...
      handleUpdate(index, row) {
        ...
        this.admin.password = null; //密码设为空
      },
      handleDialogConfirm() {
        ...
        getRoleByAdmin(adminId).then(response => {
          ...
          if(allocRoleList!=null&&allocRoleList.length>0){
            for(let i=0;i<allocRoleList.length;i++){
              ...
              this.allocRoleIds.push(allocRoleList[i]);
            }
          }
        });
      }
